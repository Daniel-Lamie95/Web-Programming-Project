...
if ($name === '' || $password === '' || ($userType !== 'student' && $userType !== 'company')) {
	header('Location: login.html?error=missing_fields');
	exit();
}

if ($userType === 'student') {
	$sql = 'SELECT * FROM Student WHERE name = ? AND password = ? LIMIT 1';
	...
	if ($result && mysqli_num_rows($result) === 1) {
		...
		$_SESSION['user_name'] = isset($student['name']) ? $student['name'] : $name;
...
if ($userType === 'company') {
	...
	if ($result && mysqli_num_rows($result) === 1) {
		...
		$_SESSION['user_name'] = isset($company['name']) ? $company['name'] : $name;
